...
def is_adjacent(num_pos: tuple[int, int], symbol_pos: int) -> bool:
    return ((num_pos[0] <= symbol_pos and num_pos[1] > symbol_pos) or 
            num_pos[1] == symbol_pos or num_pos[0] == symbol_pos + 1)
...
def get_sum_of_adjacent_numbers(adjacent_numbers: List[int]) -> int:
    return reduce((lambda x, y: x + y), adjacent_numbers) if len(adjacent_numbers) > 0 else 0

def get_product_of_adjacent_gears(adjacent_numbers: List[int]) -> int:
    return reduce((lambda x, y: x * y), adjacent_numbers) if len(adjacent_numbers) == 2 else 0
...
def main():
    ...
    print(f'Task 2: The sum of all of the gear ratios is {get_sum_of_all_gear_ratios(lines)}')
