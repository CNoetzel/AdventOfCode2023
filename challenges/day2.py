...
def get_sum_of_possible_games(data: List[str]):
    ...
    for line in data:
        ...
        sum += (
            int(splitted[0].replace("Game ", ""))
            if is_game_possible(splitted[1])
            else 0
        )
    return sum
...
def main():
    ...
    print(
        f"Task 1: The sum of IDs of possible games is {get_sum_of_possible_games(lines)}"
    )
