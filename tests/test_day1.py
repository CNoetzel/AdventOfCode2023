from challenges import day1

def test_task1():
    input_task_1 = ['1abc2', 'pqr3stu8vwx', 'a1b2c3d4e5f', 'treb7uchet']
    result = day1.sum_calibration_values(input_task_1, day1.get_calibration_value)
    assert result == 142
    
def test_task2():
    input_task_2 = ['two1nine', 'eightwothree', 'abcone2threexyz', 'xtwone3four', '4nineeightseven2', 'zoneight234', '7pqrstsixteen']
    result = day1.sum_calibration_values(input_task_2, day1.get_calibration_value_advanced)
    assert result == 281
